...
async function seed() {
    let pool;
    try {
        // 1) Read seed data from JSON
        const data = JSON.parse(await fs.readFile("./db/eventHallsData.json", "utf-8"));

        // 2) Connect to MySQL (no database selected yet)
        ...
        console.log("⚠️ Dropping & recreating database:", dbName);
    ...
    `);
        ...
        for (const catering of data.catering_options || []) {
            await pool.query(
